...
def get_recommendations(user_cluster):
    centroids = pd.read_csv('../data/processed/centroids.csv',
                            index_col=['Unnamed: 0'])
    item_factors_unstacked = pd.read_csv('../data/processed/\
                                         item_factors_unstacked.csv',
                                         index_col=['id'])
    ...
    for index, cluster in enumerate(near_clusters):
        if index == 0:
            recs = np.random.choice(top_rated_movies(cluster, centroids,
                                                     item_factors_unstacked),
                                    size=6,
                                    replace=False)
            recommendation_set.update(set(recs))
        if index == 1:
            cluster_unique_top_movies = set(top_rated_movies(cluster,
                                                             centroids,
                                                             item_factors_unstacked))\
